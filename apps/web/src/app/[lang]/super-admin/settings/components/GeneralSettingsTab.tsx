        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Excluded test accounts (user IDs)
            </label>
            <input
              type="text"
              value={settings.financialExcludedUserIds}
              onChange={(e) => updateSettings({ financialExcludedUserIds: e.target.value })}
              placeholder="e.g. 14, 27"
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
            />
            <p className="text-xs text-gray-500 mt-1">
              Comma-separated user IDs left out of every financial report. Use for test/demo accounts.
            </p>
          </div>
        </div>
      </div>
